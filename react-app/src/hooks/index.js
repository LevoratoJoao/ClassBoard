export { useAlunoData } from "./useAlunoData";
export { useAlunoFilters } from "./useAlunoFilters";
export { useRanking } from "./useRanking";
export { useAlunosList } from "./useAlunosList";

export { default as useAlunoDataDefault } from "./useAlunoData";
export { default as useAlunoFiltersDefault } from "./useAlunoFilters";
export { default as useRankingDefault } from "./useRanking";
export { default as useAlunosListDefault } from "./useAlunosList";
