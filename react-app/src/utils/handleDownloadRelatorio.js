...
import { buildAlunoAiAnalysis, buildMateriaAiAnalysis } from "../services/aiService";
import { getAllAlunos } from "../services/alunosService";

export const handleDownloadRelatorio = async () => {
  const alunos = getAllAlunos();
  const materias = [
    "Matematica", "Portugues", "Historia", "Geografia", "Ciencias", "Artes"
  ];
  ...
  alunos.forEach(aluno => {
    ...
    const analysis = buildAlunoAiAnalysis(aluno.nome);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(11);
    let lines = doc.splitTextToSize(`Resumo: ${analysis.summary}`, maxWidth - 6);
    ...
    analysis.comment.split('<br>').forEach(c => {
      ...
      if (cleanLine) {
        commentLines = commentLines.concat(doc.splitTextToSize(cleanLine, maxWidth - 6));
      }
    });
